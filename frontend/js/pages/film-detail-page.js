...
export async function filmDetailPage(app, id) {
  const cached = getFilm(id);
  app.innerHTML = '<section class="card"><h2>Loading film…</h2></section>';

  try {
    const [moviePayload, reviewPayload] = await Promise.all([
      /^\d+$/.test(String(id)) ? apiFetch(`/movies/${encodeURIComponent(id)}`) : Promise.resolve(null),
      fetchMovieReviews(id),
    ]);

    const film = moviePayload ? normalizeTmdbFilm(moviePayload) : cached;
    ...
    const backdropStyle = film.backdrop ? ` style="background-image: linear-gradient(to bottom, rgba(20,24,28,0.55) 0%, rgba(20,24,28,0.92) 100%), url('${escapeHtml(film.backdrop)}')"` : '';

    app.innerHTML = `<article class="card film-page-hero"${backdropStyle}>
      <div class="film-layout">
        ...
        <div class="film-meta film-meta-rich">
          ...
          <h2 class="page-title film-title">${escapeHtml(film.title)} <span class="muted">(${escapeHtml(film.year)})</span></h2>
          <p class="film-director-line">Directed by ${
            String(film.director || '').toLowerCase() === 'unknown'
              ? '<span class="muted">Unknown</span>'
              : film.director
                  .split(',')
                  .map((name) => renderSearchLink(name))
                  .filter(Boolean)
          ...
          <div class="film-stat-row">
            ...
            <span class="stat-pill">⏱ ${escapeHtml(film.runtime)} min</span>
          </div>
          <p class="film-overview">${escapeHtml(film.overview || 'No overview available.')}</p>
          <div class="cluster film-actions">
            <button class="primary" data-action="toggle-watchlist" data-film="${escapeHtml(film.id)}">${inWatchlist ? '✓ In watchlist' : '+ Add to watchlist'}</button>
          </div>
        </div>
      </div>
    </article>
    ...
    <section class="section">
      ...
      <div class="film-card-meta">
        ...
        <span class="meta-chip">Original language: ${escapeHtml(String(film.originalLanguage || '—').toUpperCase())}</span>
    ...
    <section class="section">
      ...
      ${renderReviewList(reviews, { currentUsername: state.currentUser?.username, movieId: film.id })}
    </section>`;
  } catch (err) {
